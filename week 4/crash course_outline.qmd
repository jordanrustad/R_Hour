...
## I want to write my thesis in R....

## What does that even mean?? Where do I begin?

Learning to code in R is a daunting enough task, and hearing words like 'repo' and 'yaml' is enough to make your head spin when you are starting out. The purpose of this R-hour (and this document) is to walk you through the steps you would need to write your thesis in R and how you can use GitHub to version your code. The very broad focus is reproducibility (could someone repeat your steps in a different study) and replicability (could someone repeat your study and get similar results). If you miss R hour, there should be enough information here that you can walk through the steps yourself. We will be linking R-Studio with Zotero to insert citations, and GitHub so you can do everything through R-Studio. I have provided alternatives with Shiny apps/GUIs when possible. I will also show you how to use GitHub as a collaborative tool so at the very least you can share your code with your supervisor or collaborators. Make sure you complete the pre-session tasks before coming to R-hour.
...
-   Download R Studio and a markdown package:
    ...
    -   **rmarkdown** - download as a package; you will need to download extra packages to rend documents
    ...
    -   There are many types of markdown files so you can shop around and pick one, but this tutorial will focus on 'rmd' and 'qmd' files

-   Reference Manager

    -   Download Zotero and add at least one paper in your library

    -   Instructions: (From LDP 2022 Course Materials), \[edits in square brackets\]
        ...
        -   **C. Zotero and Zotfile reference management software**
            ...
            -   Set up a [Zotero account](https://www.zotero.org/user/login/)
                ...
                -   [Zotero plug-in for RStudio](https://blog.rstudio.com/2020/11/09/rstudio-1-4-preview-citations/):
                    ...
                    -   RStudio has native integration with Zotero as part of its Visual Markdown editor (requires RStudio v. 1.4 and later) \[so you don't need to load it into R like you do with GitHub\]

                    -   works without the BetterBibTeX plugin, but installing BetterBibTex can permit more control over item citation keys

                    -   We will learn more about working with [citations in Markdown](https://bookdown.org/pdr_higgins/rmrwr/adding-citations-to-your-rmarkdown.html) documents in Session 4 \[You can follow this tutorial to get started on adding in citations but don't feel like you have to\]
    ...
    -   Repo = repository, specifically where you are storing code for a given project in GitHub
...
**Note:** if you have already started a project but forgot to turn on renv, you can create a renv file by calling 'renv::init'. The renv files will save in your base directory file.
...
This is called a 'yaml' header (sometimes called 'yet another markdown language'). This is where you can specify things about your document. You can add a table of contents, what file to use as a reference for bibliography, how to number your pages, what font to use, and more.

In this markdown document, type something, anything. 'It's my first markdown doc' is an uninspiring option.

Now we are going to create a citation. Type something about the paper you would like to cite. Then insert the in-text citation. To do this you can use the '\@' symbol \[to call a drop box of your papers in Zotero\] or go to Insert - Citation (along the top bar of your Rmd doc). A pop up window will ask you what format you want your citations to be in, and if you want to generate a '.bib' file. Click yes to the bib file. This is where R will pull the full citations from when you render your document. You won't see the full bibliography until you render the document.
