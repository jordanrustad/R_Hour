...
#Analyses
#Amazing resource with code on a multitude of different stats tests: https://stats.oarc.ucla.edu/other/dae/
...
#There must be NAs in the dataset somewhere and so we need to tell the cor procedure to ignore them
...
#REGRESSION
reg_example<-lm(Life.expectancy~Hepatitis.B+Measles+BMI, data=lifedata) #almost all models follow the same "setup" 
summary(reg_example)
...
#Visualize quickly what the model is predicting (not great for publication graphs, but good for quick "looks" at the result)
library(visreg)
BMI_plot<-visreg(reg_example,"BMI") #holds every other variables at average/mean values but changes BMI
plot(BMI_plot)
HepB_plot<-visreg(reg_example,"Hepatitis.B") #holds every othr variable at average values, but changes Hepatitis.B
...
library(lme4)
regmixed_example<-lmer(Life.expectancy~Hepatitis.B+Measles+BMI +(1|Country), data=lifedata) #almost all models follow the same "setup", so knowing how to set up the basic regression will help with everything else
